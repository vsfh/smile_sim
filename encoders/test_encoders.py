...
from torch.nn import Linear, Conv2d, PReLU, Sequential, Module, InstanceNorm2d
import math
import sys
sys.path.append('.')
sys.path.append('..')

from encoders.test_helpers import get_blocks, Flatten, bottleneck_IR, bottleneck_IR_SE
...
class LightStyleBlock(Module):
    def __init__(self, in_c, out_c, spatial, max_pooling=False):
        super(LightStyleBlock, self).__init__()
        self.out_c = out_c
        self.spatial = spatial
        self.max_pooling = max_pooling
        num_pools = int(np.log2(spatial))
...
class ResnetGenerator(nn.Module):
    def __init__(self, in_channel=19, res_num=2):
        ...
        modules.append(InstanceNorm2d(64))  
...
class GradualStyleEncoder(Module):
    def __init__(self, num_layers, mode='ir', input_nc=3, res_num=2, output_size=256, use_skip=False, use_skip_torgb=False, use_att=0):
        ...
        self.style_count = int(math.log(output_size, 2)) * 2 - 2
        self.coarse_ind = 3
        self.middle_ind = 7
        ...
        if use_skip: ##### modified
            ...
            for inc, outc in channels[:7]: # from 4 to 256
                self.fusion.append(FusionLayer(inc, outc, use_skip_torgb, use_att))
...
class LightStyleEncoder(Module):
    def __init__(self, num_layers, mode='ir', input_nc=3, res_num=2, output_size=256, use_skip=False, use_skip_torgb=False, use_att=0):
        
        super(LightStyleEncoder, self).__init__()
        assert num_layers in [50, 100, 152], 'num_layers should be 50,100, or 152'
        assert mode in ['ir', 'ir_se'], 'mode should be ir or ir_se'
        blocks = get_blocks(num_layers)
        if mode == 'ir':
        ...
        # for i in range(self.style_count):
        #     if i < self.coarse_ind:
        #         style = LightStyleBlock(512, 512, 16)
        #     elif i < self.middle_ind:
        #         style = LightStyleBlock(512, 512, 32)
        #     else:
        #         style = LightStyleBlock(512, 512, 64)
        #     self.styles.append(style)
        self.latlayer1 = nn.Conv2d(256, 512, kernel_size=1, stride=1, padding=0)
        self.latlayer2 = nn.Conv2d(128, 512, kernel_size=1, stride=1, padding=0)
    ...
    # feats[0] is f for the 1st conv layer, feats[1] is f for the 1st torgb layer
    # feats[2-8] is the skipped encoder features 
    def forward(self, x, styles=None, return_feat=False, return_full=False, input_edge=False): ##### modified      
        if x.shape[1] != 3:
            x = self.input_label_layer(x)
        else:
            x = self.input_layer(x)
        ...
        if styles is not None:
            for j in range(self.coarse_ind):
                latents.append(styles[j](c3))

            p2 = self._upsample_add(c3, self.latlayer1(c2))
            for j in range(self.coarse_ind, self.middle_ind):
                latents.append(styles[j](p2))

            p1 = self._upsample_add(p2, self.latlayer2(c1))
            for j in range(self.middle_ind, self.style_count):
                latents.append(styles[j](p1))

            out = torch.stack(latents, dim=1)
        
        if not return_feat and styles is not None:
            return out
