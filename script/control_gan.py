...
from stylegan2.model import ConvLayer, ResBlock, EqualLinear, PixelNorm, StyledConv, ConstantInput, ToRGB
...
class ControlNet(nn.Module):
    def __init__(self, size, input_channel, latent=256, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
...
class ControlModel(nn.Module):
    def __init__(
        ...
        generator = Generator(size,
                            style_dim,
                            n_mlp,
                            channel_multiplier=channel_multiplier).cuda()
        if not path is None:
            generator.load_state_dict(torch.load(path)["g_ema"])
        control_net = ControlNet(size, input_channel=3)
...
if __name__ == '__main__':
    # model = Generator(256, 256, 8).cuda()
    # sample = torch.randn(4,256).cuda()
    # model([sample])
    # ControlNet(256, 3)
    model = ControlModel(256,512,8,export=True).cuda()
    model.load_state_dict(torch.load('/mnt/share/shenfeihong/weight/smile-sim/2023.5.26/030000.pt')["g_ema"])
    out = model(torch.randn(1,3,256,256).cuda())
