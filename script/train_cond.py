...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        fake_img, _ = generator(noise, cond_img)
        ...
        # g_loss = F.mse_loss(fake_img, real_img) # modify
        ...
        if False:
            ...
            path_cond_img = cond_img[:path_batch_size].detach().clone()
        ...
        if i % 100 == 0:
            with torch.no_grad():
                # g_ema.eval()
                # sample, _ = g_ema([sample_z], cond_img)
                utils.save_image(
                    fake_img.detach(),
                ...
                utils.save_image(
                    cond_img[:,:3,:,:],
                    f"{args.weight_dir}/sample/cond_{str(i).zfill(6)}.png",
                    nrow=int(args.batch ** 0.5),
                    normalize=True,
                    range=(0, 1),
                )
        if i % 10000 == 0:
            torch.save(
                ...
                f"{args.weight_dir}/checkpoint/{str(i).zfill(6)}.pt",
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", type=str, default='/ssd/gregory/smile/YangNew', help="path to the lmdb dataset")
    ...
    parser.add_argument(
        "--augment", default=False, help="apply non leaking augmentation"
    )
    parser.add_argument(
        "--augment_p",
        type=float,
    ...
    if args.arch == 'stylegan2':
        from stylegan2.model_cond import pSp as Generator
        from stylegan2.model_cond import Discriminator
