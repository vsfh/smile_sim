...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        if args.augment:
            ...
            cond_img_aug, _ = augment(cond_img, ada_aug_p)
            
            fake_img, _ = augment(fake_img, ada_aug_p)

        else:
            real_img_aug = real_img
            cond_img_aug = cond_img
        ...
        cond_img = cond_img.detach().clone()
        fake_img, _ = generator(noise, cond_img)
        ...
        if g_regularize:
            ...
            path_cond_img = cond_img.detach().clone()[:path_batch_size]
        ...
        if i % 100 == 0:
            with torch.no_grad():
                g_ema.eval()
                sample, _ = g_ema([sample_z], cond_img.detach().clone())
                utils.save_image(
                    sample,
