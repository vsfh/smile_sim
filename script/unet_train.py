...
def train_control(args, loader, discriminator, device):
    ...
    if args.train_gan:
        ...
        # params += list(model.generator.to_rgbs.parameters())
    ...
    for idx in pbar:
        ...
        edge = real_batch['edge'].to(device)
        cond = real_img*(1-mask) + edge*mask
        
        opt.zero_grad()
        pred = model(noise, edge)
        # break
        loss = torch.nn.functional.mse_loss(real_img*mask, pred*mask)+F.softplus(discriminator(pred)).mean()
        loss.backward()
        ...
        if not i % 500:
            ...
            utils.save_image(
                cond,
                f"{sample_dir}/{str(i).zfill(6)}_edge.png",
                nrow=int(args.n_sample ** 0.5),
                normalize=True,
                range=(0, 1),
            )
            torch.save(model.state_dict(), os.path.join(weight_dir, f'cond_{i}.pt'))
    return
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        #     fake_img, latents = generator(noise, cond, return_latents=True)
        ...
        if get_rank() == 0:
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    utils.save_image(
                        cond,
                        f"{sample_dir}/{str(i).zfill(6)}_cond.png",
                        nrow=int(args.n_sample ** 0.5),
                        normalize=True,
                        range=(0, 1),
                    )
...
if __name__ == "__main__":
    ...
    def get_args():
        ...
        args.latent = 512
        args.n_mlp = 8
        args.weight_dir = '/data/shenfeihong/share/weight/smile/2023.7.5'
        args.start_iter = 0
        args.project = True
        return args

    args = get_args()
    from control_gan import Discriminator
    from denoising_model.unet import Unet

    generator = Unet(
        256, dim_mults=(1,2,2,2)
    ).to(device)
    if args.project:
        input_channel = 4
    else:
    ...
    # train_control(args, loader, discriminator, device)